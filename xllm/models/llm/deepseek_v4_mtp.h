...
class DeepseekV4MultiTokenPredictorLayerImpl
 ...
 public:
  ...
  torch::Tensor forward(torch::Tensor inputs_embeds,
                        ...
                        const layer::Dsv4CpMetadata* cp_metadata) {
    ModelInputParams modified_input_params = input_params;
    modified_input_params.embedding.input_embedding = previous_hidden_states;
    std::optional<torch::Tensor> residual;
    auto decoder_forward = [cp_metadata](
                               layer::DeepseekV4DecoderLayer& decoder,
    ...
    return forward_with_decoder(std::move(inputs_embeds),
                                residual,
                                std::move(positions),
                                attn_metadata,
                                kv_cache,
 ...
 public:
  explicit DeepseekV4MtpModelImpl(const ModelContext& context)
    ...
    CHECK(parallel_args.tp_group_ != nullptr)
        << "DeepSeek V4 MTP requires an attention TP process group";
  ...
  ModelOutput forward(torch::Tensor tokens,
    ...
    if (cp_plan.enabled()) {
      CHECK(!acl_graph_forward)
          << "DeepSeek V4 MTP CP prefill must execute outside ACL graph";
      ...
      cp_inputs.hidden_states = &hidden_states;
      cp_inputs.positions = &positions;
      cp_inputs.token_ids = &layer_tokens;
      cp_inputs.auxiliary_hidden_states = &previous_hidden_states;
    ...
    torch::Tensor flat_aux_hidden_states = aux_hidden_states.flatten(1);
    if (cp_plan.enabled()) {
      std::vector<torch::Tensor> global_outputs =
          cp_plan.row_layout().gather_global_rows_bundle(
              {output.contiguous(), flat_aux_hidden_states.contiguous()},
              cp_plan.process_group());
      CHECK_EQ(global_outputs.size(), 2);
      output = std::move(global_outputs[0]);
      flat_aux_hidden_states = std::move(global_outputs[1]);
    }
    return ModelOutput(
        output, torch::Tensor(), std::move(flat_aux_hidden_states));
  }
