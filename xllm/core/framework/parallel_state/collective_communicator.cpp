...
void CollectiveCommunicator::create_process_groups(
  ...
  if (::xllm::KernelConfig::get_instance().npu_kernel_backend() == "TORCH" &&
      cp_size > 1) {
    CHECK_EQ(tp_size * dp_size * cp_size, world_size);
  } else {
  ...
  if (::xllm::KernelConfig::get_instance().npu_kernel_backend() == "TORCH" &&
    ...
    CHECK_EQ(cp_ranks.size(), cp_size);
    CHECK_EQ(cp_ranks[cp_local_rank], global_rank);
    const int32_t dp_rank = global_rank / (cp_size * tp_size);
    const int32_t tp_rank = global_rank % tp_size;
    const int32_t cp_group_index = dp_rank * tp_size + tp_rank;
    cp_group_count = dp_size * tp_size;
    const int32_t cp_port_base = port + tp_group_count +
                                 single_rank_group_port_gap +
                                 single_rank_group_count;
    const std::string cp_host =
...
#else
  // The current MLU model-side CP path spans the DP-local TP group.
  parallel_args_->cp_group_ = tp_group_.get();
#endif
  port += tp_group_count + single_rank_group_port_gap +
          single_rank_group_count + cp_group_count;

  if (dp_size > 1) {
    const int32_t dp_group_count = world_size / dp_size;
    const int32_t dp_group_index = global_rank % dp_group_count;
    port_offset = dp_group_index + 1;
    const std::string dp_host =
        get_rank_table_server_host(dp_group_index, host);
