...
MoeRowCapability FusedMoEImpl::row_capability(
  ...
  return can_use_ep2_dispatch_combine(input_params, hidden_states)
             ...
             : MoeRowCapability::REQUIRES_IDENTICAL_GLOBAL_ROWS;
}
...
torch::Tensor FusedMoEImpl::forward_with_selected_experts_ep2(
  ...
  if (input_params.parallel.cp_plan.enabled() ||
    ...
    // Explicit multiplication is wrong for uneven DP or CP-local batches.
    global_bs = 0;
  }
...
torch::Tensor FusedMoEImpl::forward_with_selected_experts(
    ...
    const torch::Tensor& topk_ids,
    const ModelInputParams& input_params,
    std::optional<MoeRowCapability> expected_row_capability,
    MoeRowExecutionMode execution_mode) {
  torch::Tensor input = hidden_states;
  torch::Tensor selected_topk_weights = topk_weights;
  torch::Tensor selected_topk_ids = topk_ids;
  const MoeRowCapability current_row_capability =
      row_capability(input_params, input);
  if (expected_row_capability.has_value()) {
    CHECK(current_row_capability == expected_row_capability.value())
        << "MoE row capability changed after CP row preparation";
  }
  const bool use_local_rows =
      execution_mode == MoeRowExecutionMode::LOCAL_ROWS ||
      (execution_mode == MoeRowExecutionMode::AUTO &&
       current_row_capability == MoeRowCapability::PRESERVES_LOCAL_ROW_ORDER);
  if (use_local_rows) {
    CHECK(current_row_capability == MoeRowCapability::PRESERVES_LOCAL_ROW_ORDER)
        << "Local-row MoE execution requires dispatch/combine capability";
    std::optional<torch::Tensor> shared_output = std::nullopt;
    if (n_shared_experts_ > 0) {
      shared_output = shared_experts_(input);
      if (should_apply_shared_expert_gate(shared_expert_gate_,
