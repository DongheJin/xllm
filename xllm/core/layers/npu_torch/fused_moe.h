...
enum class MoeRowExecutionMode : int8_t {
  AUTO = 0,
  GLOBAL_ROWS = 1,
  LOCAL_ROWS = 2,
};
...
class FusedMoEImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor forward_with_selected_experts(
      ...
      std::optional<MoeRowCapability> expected_row_capability = std::nullopt,
      MoeRowExecutionMode execution_mode = MoeRowExecutionMode::AUTO);
  torch::Tensor forward(const torch::Tensor& hidden_states,
                        const ModelInputParams& input_params);
  MoeRowCapability row_capability(const ModelInputParams& input_params,
