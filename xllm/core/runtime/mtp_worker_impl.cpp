...
void validate_dsv4_mtp_state_dtype(const ModelArgs& target_args,
                                   const ModelArgs& draft_args) {
  const bool target_is_dsv4 =
      util::is_deepseek_v4_model_type(target_args.model_type());
  const bool draft_is_dsv4 =
      util::is_deepseek_v4_model_type(draft_args.model_type());
  if (!target_is_dsv4 && !draft_is_dsv4) {
    return;
  }
  CHECK(target_is_dsv4 && draft_is_dsv4)
      << "DSV4 MTP target and draft must use the same compressor state ABI";
  const auto state_dtype = parse_dsv4_compress_state_dtype(
      ::xllm::KVCacheConfig::get_instance().dsv4_compress_state_dtype());
  CHECK(state_dtype.has_value())
      << "DSV4 MTP target and draft require a valid compressor state dtype";
  LOG(INFO) << "[DSV4][MTP][KVCacheInit] target/draft compressor state dtype="
            << dsv4_compress_state_dtype_name(*state_dtype);
}
...
void check_mtp_cp_row_layout(const ForwardInput& reference,
                             const ForwardInput& candidate) {
#if defined(USE_NPU)
  const NpuCpPlan& reference_plan = reference.input_params.parallel.cp_plan;
  const NpuCpPlan& candidate_plan = candidate.input_params.parallel.cp_plan;
  CHECK(reference_plan.has_same_row_layout(candidate_plan))
      << "MTP target and draft must use the same CP row ownership: target_cp="
      << reference_plan.enabled() << ", draft_cp=" << candidate_plan.enabled()
      << ", target_signature=" << reference_plan.row_layout().signature()
      << ", draft_signature=" << candidate_plan.row_layout().signature();
#else
  UNUSED_PARAMETER(reference);
  UNUSED_PARAMETER(candidate);
#endif
}
...
std::optional<ForwardOutput> run_llm_no_sync_impl(
    ...
    const ForwardInput* cp_layout_reference = nullptr) {
  worker.prepare_work_before_execute_on_stream(
      input,
      processed_input,
      prepare_stream,
      /*record_ready_event=*/&prepare_stream != &compute_stream);
  if (cp_layout_reference != nullptr) {
    check_mtp_cp_row_layout(*cp_layout_reference, processed_input);
  }
...
Stream& MTPWorkerImpl::mtp_prepare_stream() {
  return mtp_prepare_stream_ != nullptr ? *mtp_prepare_stream_
                                        : *prepare_stream_;
}
...
#if defined(USE_NPU)
    if (mtp_prepare_stream_ == nullptr &&
        mtp_async::requires_isolated_metadata_stream(
            context_.get_model_args().model_type(),
            enable_schedule_overlap(),
            parallel_args_.cp_size())) {
      mtp_prepare_stream_ = device_.get_stream_from_pool();
    }
#endif
  }
...
bool MTPWorkerImpl::allocate_kv_cache(const KVCacheShape& kv_cache_shape) {
  ...
  CHECK(impl_ != nullptr);
  CHECK(draft_impl_ != nullptr);
  validate_dsv4_mtp_state_dtype(impl_->context_.get_model_args(),
...
std::optional<ForwardOutput> MTPWorkerImpl::step_empty(
  ...
  if (!input.input_params.meta.batch_forward_type.is_decode()) {
    ...
    auto output = run_llm_no_sync_impl(
        *impl_, input, mtp_prepare_stream(), *compute_stream_, target_prepared);
...
std::optional<ForwardOutput> MTPWorkerImpl::step_prefill(
...

  {
    ...
    if (should_broadcast_spec_tokens(
      ...
      broadcast_mtp_tokens(output.sample_output.next_tokens,
                           parallel_args_,
                           /*root_rank=*/0);
    }
...
void MTPWorkerImpl::prepare_prefill_inputs(const ForwardInput& input,
                                           ForwardInput& prefill_input) {
  Stream& prepare_stream = mtp_prepare_stream();
  ...
  finish_metadata_prepare(prepare_stream, prefill_input);
}
  ...
  for (int32_t draft_idx = 0; draft_idx < num_speculative_tokens; ++draft_idx) {
    ...
    draft_outputs.emplace_back(std::move(draft_output_opt.value()));
    {
      ...
      if (should_broadcast_spec_tokens(
        ...
        broadcast_mtp_tokens(draft_sample.next_tokens,
                             parallel_args_,
                             /*root_rank=*/0);
      }
...
std::optional<ForwardOutput> MTPWorkerImpl::run_validate(
  ...
  StreamEventPtr target_context_ready_event;
  {
    ...
    if (should_broadcast_spec_tokens(
            ...
            get_optimization_config().enable_spec_token_broadcast,
            input.sampling_params.all_greedy_sample)) {
      broadcast_mtp_tokens(val_output.next_tokens,
                           parallel_args_,
                           /*root_rank=*/0);
    }
...
#endif
  finish_metadata_prepare(prepare_stream, validate_input);
}
...
void MTPWorkerImpl::prepare_draft_extend_inputs(
  ...
  finish_metadata_prepare(prepare_stream, extend_input);
}
...
void MTPWorkerImpl::prepare_draft_inputs(const ForwardInput& input,
  ...
  finish_metadata_prepare(prepare_stream, draft_input);
}
